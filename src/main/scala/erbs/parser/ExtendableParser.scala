...
class ExtendableParser extends RubyParser with OperatorToken with ParserMap with ParserErrors {
  ...
  protected lazy val TagDefinitionArgs: PackratParser[FormalArgs] =  "(" ~> simpleArgList.? <~ ")" ^^ { args => FormalArgs(args.getOrElse(Nil)) }
  protected lazy val opTags: PackratParser[Set[String]] = TagDefinitionArgs ^^ { _.args.collect { case SimpleArgElement(v) => v.v }.toSet }
